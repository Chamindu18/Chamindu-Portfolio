...
if (!canvas) {
  console.warn('Canvas element not found, background animation skipped');
} else {
  ...
  const MOBILE_PARTICLE_COUNT = 40;    // Reduced from 100
  const DESKTOP_PARTICLE_COUNT = 100;
  let PARTICLE_COUNT = isTouchDevice ? MOBILE_PARTICLE_COUNT : DESKTOP_PARTICLE_COUNT;
  let frameInterval = isTouchDevice ? 2 : 1; // Skip every other frame on mobile (capped ~30fps)
  let frameCounter = 0;
  ...
  function regenerateParticles() {
    // Clear existing particles array and recreate with new count
    parts.length = 0;
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      parts.push(new Pt(true));
    }
  }
  ...
  // Blob animation (same as before, no changes needed)
  ...
  function render(now) {
    // Throttle frames on mobile for battery saving
    if (isTouchDevice) {
      frameCounter++;
  ...
  // Start animation after initial resize
  resize();
